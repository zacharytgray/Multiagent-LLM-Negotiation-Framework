...
class TestAgent(unittest.TestCase):
    ...
    def hasOptimalAllocation(self, agent1Tasks, agent2Tasks, optimalAgent1Tasks, optimalAgent2Tasks):
        optimalAgent1PSRs = [(PSR1, PSR2) for _, PSR1, PSR2 in optimalAgent1Tasks]
        optimalAgent2PSRs = [(PSR1, PSR2) for _, PSR1, PSR2 in optimalAgent2Tasks]
        
        # Check PSR pairs in agent1Tasks
        for _, PSR1, PSR2 in agent1Tasks:
            if (PSR1, PSR2) not in optimalAgent1PSRs:
                return False
        
        # Check PSR pairs in agent2Tasks
        for _, PSR1, PSR2 in agent2Tasks:
            if (PSR1, PSR2) not in optimalAgent2PSRs:
                return False
        return True
    ...
    def run_round(self, round_num, numRounds):
        ...
        if not hasOptimalAllocation:
            ...
            print("\nOptimal Allocation:")
            print(f"Agent 1: {optimalAllocation[0]}")
            print(f"Agent 2: {optimalAllocation[1]} {Fore.RESET}")
