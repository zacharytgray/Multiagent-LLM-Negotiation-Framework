...
class Domain:
	def __init__(self, items: list[Item], agent1Model: str, agent2Model: str) -> None:
		filePath = "CompetitiveAllocators/CompetitiveSystemInstructions1Ex.txt"
		self.agent1 = Agent("Agent 1", agent1Model, filePath, useOpenAI=False) # Agent 1 uses local model
		self.agent2 = Agent("Agent 2", agent2Model, filePath, useOpenAI=True) # Agent 2 uses OpenAI model
		self.moderatorAgent = Agent("Moderator", agent2Model, "", useOpenAI=True)
